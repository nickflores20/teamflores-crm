...
import { formatDistanceToNow, isToday } from '../../lib/dateUtils.js'
import { getFullName } from '../../api/mockData.js'

export default function ActivityItem({ lead, isSelected, isNew, onClick }) {
  ...
  return (
    <button
      ...
      className={[
        ...
        isNew && !isSelected ? 'border-l-2 border-l-gold/60 pl-[14px]' : '',
      ].filter(Boolean).join(' ')}
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <p className={`text-sm truncate ${isNew ? 'font-semibold text-ink' : 'font-medium text-ink-secondary'}`}>
            {name}
          </p>
