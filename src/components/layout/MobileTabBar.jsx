...
export default function MobileTabBar() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-surface-border h-tabbar flex items-stretch">
      {TABS.map(tab => (
        <NavLink
          key={tab.to}
          to={tab.to}
          className={({ isActive }) => [
            ...
            isActive ? 'text-navy-800' : 'text-ink-muted',
          ].join(' ')}
        >
          {({ isActive }) => (
            <>
              ...
              <span className="relative">
                ...
                {tab.to === '/inbox' && unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-1 w-2 h-2 bg-gold rounded-full" />
                )}
              </span>
