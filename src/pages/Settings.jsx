    ...
    >
      ...
      <div className="flex-1 px-4 lg:px-6 py-6 space-y-5 max-w-2xl">
        ...
        <Section title="License & Compliance" icon="🏦">
          ...
          <div>
            <p className="text-xs font-medium text-ink-secondary mb-2">Licensed States</p>
            <div className="flex flex-wrap gap-2">
              {['NV','AZ','CA','FL','TX','WA','OR'].map(state => (
                <label key={state} className="flex items-center gap-1.5 cursor-pointer select-none">
                  <span className="w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0"
                    style={{ backgroundColor: '#1A3E61', borderColor: '#1A3E61' }}>
                    <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                    </svg>
                  </span>
                  <span className="text-xs font-bold" style={{ color: '#1A3E61' }}>{state}</span>
                </label>
              ))}
            </div>
          </div>
          <div className="bg-surface-secondary rounded-xl p-3 border border-surface-border">
            <p className="text-xs font-medium text-ink mb-1">Email Signature Preview</p>
            <div className="text-xs text-ink-secondary leading-relaxed font-mono whitespace-pre-wrap">
{`Nicholas Flores
Division Director | Sunnyhill Financial
NMLS #422150
📞 702-497-6370 | 📧 nick@sunnyhillfinancial.com
Licensed: NV • AZ • CA • FL • TX • WA • OR
...
"You can't have a positive life with a negative mind."`}
        ...
        <Section title="Automation" icon="⚡">
          ...
          />
          <div>
            <label className="text-xs font-medium text-ink-secondary block mb-1.5">Speed to Lead</label>
            <select
              value={automation.speedToLead ?? 'immediately'}
              onChange={e => saveAutomation({ speedToLead: e.target.value })}
              className="w-full bg-white border border-surface-border rounded-lg px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-gold/30 focus:border-gold/50 transition-colors"
            >
            ...
            <p className="text-xs text-ink-muted mt-1">
              Initial text fires {automation.speedToLead === 'immediately' ? 'immediately' : `${automation.speedToLead ?? 'immediately'}`} after form submission
            </p>
          </div>
        </Section>
        ...
        <Section title="Integrations" icon="🔗">
          <div className="space-y-4">
            <div>
              <label className="text-xs font-medium text-ink-secondary block mb-1.5">Calendly URL</label>
              <div className="flex gap-2">
                <input
                  value={calendlyUrl}
                  onChange={e => setCalendlyUrl(e.target.value)}
                  placeholder="calendly.com/floresnick"
                  className="flex-1 bg-white border border-surface-border rounded-lg px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-gold/30 focus:border-gold/50 transition-colors"
                />
                <a
                  href={`https://${calendlyUrl}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-3 py-2 text-xs font-semibold rounded-lg border border-surface-border text-ink-secondary hover:bg-surface-secondary transition-colors whitespace-nowrap"
                >
                  Test Link ↗
                </a>
              </div>
            ...
            <div>
              <label className="text-xs font-medium text-ink-secondary block mb-1.5">Sonar Pre-Approval URL</label>
              <div className="flex gap-2">
                <input
                  value={sonarUrl}
                  onChange={e => setSonarUrl(e.target.value)}
                  placeholder="https://sunnyhillfinancial.pos.yoursonar.com/…"
                  className="flex-1 bg-white border border-surface-border rounded-lg px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-gold/30 focus:border-gold/50 transition-colors"
                />
                <a
                  href={sonarUrl}
            ...
            <div className="flex items-center justify-between py-2 border-t border-surface-border">
              <div>
                <p className="text-sm font-medium text-ink">Demo Mode (USE_MOCK)</p>
                <p className="text-xs text-ink-muted mt-0.5">All data served from local mock — no Google Sheets connection</p>
              </div>
              <span className="text-xs font-bold px-2 py-1 rounded-full bg-amber-100 text-amber-700">ON</span>
            </div>
          </div>
        </Section>
